...
Returns any number of track lines:
  ...
  wwr_req_recur("40001") \t tracknumber \t trackname \t trackflags \t volume \t pan \t last_meter_peak \t last_meter_pos \t width/pan2 \t panmode \t sendcnt \t recvcnt \t hwoutcnt \t color
...
export function wwr_run_update()
{
  g_wwr_timer = null;
...
export function wwr_start() { wwr_run_update(); }
export function wwr_req(name) { g_wwr_req_list += name + ";"; }
export function wwr_req_recur(name, interval) { g_wwr_req_recur.push([name,interval,0]); }
export function wwr_req_recur_cancel(name) { 
  var i;
...
export function simple_unescape(v) {
